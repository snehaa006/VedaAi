...
function getOpenAIImageContent(uploadedFile?: IUploadedFile) {
  if (!uploadedFile) return null;
  if (!SUPPORTED_IMAGE_TYPES.has(uploadedFile.mimeType)) return null;
  if (!uploadedFile.data) return null;
...
async function generateWithAnthropic(input: GenerationInput, prompt: string) {
...
async function generateWithOpenAI(input: GenerationInput, prompt: string) {
...
function parseQuestionPaper(rawText: string, input: GenerationInput): {
...
function buildAnswerKey(questionPaper: IQuestionPaper) {
...
function getReferenceSnippets(content?: string): string[] {
  ...
  if (conceptSnippets.length >= 3) return conceptSnippets;
  ...
  if (labelledSnippets.length >= 3) return labelledSnippets;
  ...
  for (let i = 0; i < words.length; i += 18) {
    ...
    if (chunk.length >= 20) chunks.push(truncateAtWord(chunk, 220));
...
function getTrafficDemandConcepts(content: string): string[] {
...
function cleanReferenceSnippet(snippet: string): string {
  return snippet
    .replace(/^Text extracted from uploaded image "[^"]+":\s*/i, '')
...
function getQuestionFocus(snippet: string): string {
...
function summarizeReferenceFact(snippet: string): string {
  const cleaned = cleanReferenceSnippet(snippet);
  const normalized = cleaned.toLowerCase();
...
function makeQuestionSentence(action: string, focus: string): string {
  return `${action} ${focus.charAt(0).toLowerCase()}${focus.slice(1)}.`;
}

function buildDistractors(correctAnswer: string, subject: string): string[] {
...
function buildShortReferenceQuestion(snippet: string, index: number): string {
...
function buildFillReferenceQuestion(snippet: string): string {
  const normalized = snippet.toLowerCase();
...
function buildNumericalReferenceQuestion(snippet: string): string {
  const normalized = snippet.toLowerCase();
...
function buildDiagramReferenceQuestion(snippet: string): string {
  const normalized = snippet.toLowerCase();
...
function buildReferenceAnswer(snippet: string): string {
  return summarizeReferenceFact(snippet);
}

function buildGenericQuestion(type: string, subject: string, index: number): string {
...
function extractQuotedReference(text: string): string {
...
function shouldRewriteQuestion(text: string): boolean {
  return /uploaded image|image content|Grade\s*0|Based on the|concept for Grade|shown here/i.test(text) ||
    /"[^"]{80,}"/.test(text) ||
    text.length > 260;
}

function normalizeQuestionPaper(input: GenerationInput, questionPaper: IQuestionPaper): IQuestionPaper {
  return {
    ...questionPaper,
    ...
    sections: questionPaper.sections.map((section) => ({
      ...
      questions: section.questions.map((question, index) => {
        if (!shouldRewriteQuestion(question.text)) return question;
